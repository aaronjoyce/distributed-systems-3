...
def handle_single_connection(socket, addrss):
    while True:
        ...
        if (action_name == 'CHAT'):
            ...
            room_identifier = int(data[0].split(":")[1])
            room_join_identifier = int(data[1].split(":")[1])
            message_content = data[3].split(":")[1]
            broadcast(room_identifier, "CHAT:{0}\nCLIENT_NAME:{1}\nMESSAGE:{2}\n\n".format(str(room_identifier), str(client_name), message_content))
            ...
            if room_join_identifier not in rooms[room_identifier]:
                ...
                socket.sendall("JOINED_CHATROOM:{0}\nSERVER_IP:{1}\nPORT:{2}\nROOM_REF:{3}\nJOIN_ID:{4}\n".format(str(room_name), "10.62.0.156", 8000, str(room_identifier), str(room_join_identifier)))
            ...
            room_join_identifier = int(data[1].split(":")[1])
            client_name = data[2].split(":")[1]
            socket.sendall("LEFT_CHATROOM:{0}\nJOIN_ID:{1}\n".format(str(room_identifier), str(room_join_identifier)))
            broadcast(room_identifier, "CHAT:{0}\nCLIENT_NAME:{1}\nMESSAGE:{2}\n\n".format(str(room_identifier), str(client_name), str(client_name) + " has left this chatroom."))
            del rooms[room_identifier][room_join_identifier]
        elif (action_name == 'DISCONNECT'):
            room_join_identifier = int(md5(client_name).hexdigest(), 16)
            client_name = data[2].split(":")[1]
            for room_identifier in rooms.keys():
                if room_join_identifier in rooms[room_identifier]:
                    broadcast(room_identifier, "CHAT:{0}\nCLIENT_NAME:{1}\nMESSAGE:{2}\n\n".format(str(room_identifier), str(client_name), str(client_name) + " has left this chatroom."))
                    if room_join_identifier in rooms[room_identifier]:
                        del rooms[room_identifier][room_join_identifier]
            break

def broadcast(room_identifier, data):
    for room_join_identifier, connection in rooms[room_identifier].iteritems():
        connection.sendall(data)

incoming_connections = Queue.Queue(maxsize=100)
rooms = collections.OrderedDict()
sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
sock.bind(('0.0.0.0', 8012))
sock.listen(5)
